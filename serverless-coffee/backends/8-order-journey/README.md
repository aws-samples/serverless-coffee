...
```bash
order-journey-service$ sam build
```
...
```bash
order-journey-service$ sam logs -n StockCheckerFunction --stack-name order-journey-service --tail
```
...
```bash
order-journey-service$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name order-journey-service
```
